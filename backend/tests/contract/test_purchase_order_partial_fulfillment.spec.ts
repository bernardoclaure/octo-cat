...
describe('Purchase order partial fulfillment contract', () => {
  beforeEach(() => {
  ...
  it('records partial fulfillment and exposes fulfillment history', async () => {
    ...
    const fulfillResponse = await request(app)
      ...
      .send({
        lineItemFulfillments: [{ lineItemId, quantity: 1, shipmentReference: 'SHIP-001' }],
      })
      .expect(200);

    expect(fulfillResponse.body.status).toBe('Partially Fulfilled');
    expect(fulfillResponse.body.lineItems[0].fulfilledQuantity).toBe(1);
    ...
    expect(historyResponse.body[0].events).toHaveLength(1);
